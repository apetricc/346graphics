...
function render()
{
    gl.clear( gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);   //bitwise OR 

    theta[axis] += 2.0; //rotate by 2 degrees
    gl.uniform3fv(thetaLoc, theta); //find theta in html & set it 

    gl.drawElements(gl.TRIANGLES, elementCount, gl.UNSIGNED_SHORT, 0);  //draw out elements (triangles)...elementCount--# indices

    requestAnimFrame( render ); 
    //calling render for the frame refresh, or when the frame refresh is made... don't call render() that would be an infinite loop
}
